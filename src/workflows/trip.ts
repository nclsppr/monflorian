  ...
  async run(event: WorkflowEvent<TripWorkflowParams>, step: WorkflowStep) {
    ...
    try {
      ...
      if (!itineraryStep.hasPhotos) {
        return { status: itineraryStep.status, tripId, itineraryProviderRequestId: itineraryStep.providerRequestId };
      }
      ...
      const imageStep = await step.do(
        ...
        async () => {
          ...
          const destination = typeof itinerary.destination === "string" ? itinerary.destination : "le voyage proposé";
          const days = Array.isArray(itinerary.days) ? itinerary.days : [];
          const firstDay = days[0] && typeof days[0] === "object" ? days[0] as Record<string, unknown> : null;
